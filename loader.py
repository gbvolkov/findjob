...
def convert_audio_to_wav(input_file: str,
                         ...
                         target_rate: int = 16_000) -> None:
    """
    Re-encode *input_file* to mono PCM WAV without spawning ffmpeg.

    PyAV (FFmpeg bindings) lets us stay entirely inside the process.
    """
    try:
        with av.open(input_file, mode="r") as container:
            stream = next((s for s in container.streams if s.type == "audio"), None)
            if stream is None:
                raise RuntimeError(f"No audio streams found in {input_file}")
            ...
            with wave.open(output_file, "wb") as wav_file:
                ...
                for frame in container.decode(stream):
                    ...
                    wav_file.writeframes(frame.planes[0].to_bytes())
    except av.AVError as err:
        raise RuntimeError(f"FFmpeg error while converting {input_file}: {err}") from err
